...
export default function Spinner() {
  ...
  const { isSpinning, finalMenuItem, setFinalMenuItem } = useFinalMenuItem();
  ...
  return (
    <div className="relative flex flex-wrap justify-center items-center h-auto">
      ...
      <div className="flex justify-end w-full">
        {!isSpinning && <h1 className={`w-[40%] xl:w-1/2 text-black absolute ml-6 bottom-[15%] hidden lg:block text-right pr-12 text-5xl xl:text-6xl ${titleAnimation} transition duration-150 ease-linear`}>{finalMenuItem?.name}</h1>}
      </div>
